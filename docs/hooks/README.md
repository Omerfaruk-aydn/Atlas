...
the agent lifecycle, allowing you to both build on top of Atlas, customize
...
- Atlas ships with a builtin `atlas-hook` skill write, edit, and configure
  hooks; just tell Atlas how to configure Atlas
- Atlas currently supports just one hook, `PreToolUse`, with plans to support
...
our **project-level** `atlas.json`. Relative paths like `./no-haskell.sh` work
here because the project root is your working directory. If you're configuring
a global hook (`~/.config/atlas/atlas.json`), use an absolute path instead.
...
# ($ATLAS_TOOL_INPUT_COMMAND) to grep and match on a regexp.
if echo "$ATLAS_TOOL_INPUT_COMMAND" | grep -qE '(^| )((ghc|cabal|stack)(\.exe)?)( |$)'; then
...
Hooks run through Atlas's embedded POSIX shell (`mvdan.cc/sh`) — the same
...
- **Permissive shebang fallback**: if the absolute path in a shebang
  doesn't exist (e.g. `#!/bin/bash` on Windows), Atlas falls back to a
  ...
  "no opinion".
- **Environment**: every hook sees `ATLAS=1`, `AGENT=atlas`, and
  `AI_AGENT=atlas` on top of the `ATLAS_*` hook-specific variables. These
  ...
  both contexts.
- **Timeout behavior**: when a hook exceeds its timeout, Atlas cancels the
  context and waits a short grace period (~1s) for the interpreter to
  yield. If the hook still hasn't returned, Atlas abandons it, logs a
...
Hooks can be added to your `atlas.json` (or `.atlas.json`) at both the global
...
> work fine in project-level `atlas.json` because the project root is also
> your working directory. For **global** config (`~/.config/atlas/`),
> however, you must use either an absolute path or an inline command:
>
> ```jsonc
> // Global ~/.config/atlas/atlas.json
> {
>   "hooks": {
>     "PreToolUse": [
>       {
>         "command": "/home/you/.config/atlas/hooks/no-haskell.sh"
...
3. Runs all matching hooks **in parallel** through Atlas's embedded POSIX
...
5. Applies the result **before** permission checks. If the aggregated decision
   ...
   for it. If it's `allow`, Atlas treats that as affirmative pre-approval and
...
however you'll incur some additional overhead as Atlas will still parse and
run each hook.
...
| ---------------------------- | ---------------------------------------------- |
| `ATLAS`                      | Always `1` when running under Atlas.           |
| `AGENT`                      | Always `atlas`.                                |
| `AI_AGENT`                   | Always `atlas`.                                |
| `ATLAS_EVENT`                | The hook event name (e.g. `PreToolUse`).       |
| `ATLAS_TOOL_NAME`            | The tool being called (e.g. `bash`).           |
| `ATLAS_SESSION_ID`           | Current session ID.                            |
| `ATLAS_CWD`                  | Working directory.                             |
| `ATLAS_PROJECT_DIR`          | Project root directory.                        |
| `ATLAS_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command being run. |
| `ATLAS_TOOL_INPUT_FILE_PATH` | For file tools: the target file path.          |

The `ATLAS`, `AGENT`, and `AI_AGENT` markers are also set by the `bash`
tool, so a script can detect "am I running under Atlas?" the same way in
either context.
...
Hooks communicate back to Atlas via **exit code** and `stdout`/`stderr`. The
...
deterministic based on the order hooks appear in `atlas.json`.
...
If a hook exceeds its timeout, Atlas cancels its context and treats the
...
if echo "$ATLAS_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
returns `decision: "allow"`, which tells Atlas to pre-approve the call:
...
case "$ATLAS_TOOL_INPUT_COMMAND" in
...
if [[ "$ATLAS_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
```jsonc
{ "command": "echo \"$(date -Iseconds) $ATLAS_TOOL_NAME\" >> ./tools.log" }
```
...
Atlas hooks are broadly compatible with [Claude Code
...
hooks run under Atlas unchanged. This document covers the Atlas-specific API
only — anything not documented here isn't guaranteed to work.

One intentional divergence: Atlas treats `updated_input` as a shallow-merge
...
4. `decision` precedence: `deny` > `allow` > `null`. First deny determines the
   ...
   is `allow`, Atlas pre-approves the tool call and skips the permission
