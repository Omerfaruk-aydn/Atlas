...
Right now `atlasrc` runs once, at startup. If you want to change something
...
- To make it stick, you edit your `atlasrc` — like editing `.bashrc`.

So you could say "Atlas, switch to the small model for a bit" and it just
...
> `atlasrc` from live config state. Want it permanent? Edit `atlasrc`.
...
Atlas currently uses JSON files in data directories as both persisted machine
state and high-priority configuration:

```text
~/.local/share/atlas/atlas.json
.atlas/atlas.json
```
...
`atlasrc` and legacy `atlas.json` files.

The goal is to make the roles explicit:

| Role | Format |
|---|---|
| User-authored executable configuration | `atlasrc` / `.atlasrc` |
| Legacy user-authored static configuration | `atlas.json` / `.atlas.json` |
| Atlas-owned persistent preferences and history | versioned `state.json` |
...
```text
~/.config/atlas/atlasrc             global user config
~/.local/share/atlas/state.json     global machine state
./atlasrc / ./.atlasrc              project user config
.atlas/state.json                   workspace machine state
```
...
→ global legacy atlas.json
→ global atlasrc
→ project legacy atlas.json
→ project atlasrc
→ project .atlas.json
→ project .atlasrc
...
3. Provide an explicit conversion command (for example,
   `atlas config convert atlas.json > atlasrc`).
...
### Typed atlasrc builder

This is related but separate. Today the Bash path is:

```text
atlasrc → map[string]any → JSON → Config
```

A later typed-builder phase should become:

```text
atlasrc → typed ConfigBuilder → Config
state.json → typed StateStore ───────┘
```

Legacy `atlas.json` would decode into a typed config patch and apply to the same
...
7. Replace the `atlasrc` map/JSON bridge with a typed config builder.
...
`atlas.json.migrated`; corrupt files should be quarantined as timestamped
...
Atlas currently has three useful tool states across both config formats:

| State | `atlasrc` | `atlas.json` | Behavior |
|---|---|---|---|
...
The one state Atlas does **not** have is "visible but always rejected": the
