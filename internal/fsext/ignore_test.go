...
func TestAtlasAgentIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .Atlasignore file that ignores .log files
	require.NoError(t, os.WriteFile(".Atlasignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .Atlasignore file
	AtlasignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".Atlasignore"), []byte(AtlasignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .Atlasignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .Atlasignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .Atlasignore in subdir that ignores normal_nested
	subAtlasignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".Atlasignore"), []byte(subAtlasignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .Atlasignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .Atlasignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .Atlasignore")
