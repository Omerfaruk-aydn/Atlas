...
// socketDir returns the directory used for the Atlas-Agent Unix socket.
...
// returned by [socketDir] and is named Atlas-Agent-<uid>.sock, falling
// back to atlas.sock when the current uid cannot be determined. If
// the composed path would exceed [maxUnixSocketPathLen] bytes (the
// macOS sun_path limit), we fall back to /tmp/Atlas-Agent-<uid>.sock so
// the socket remains bindable.
func DefaultHost() string {
	sock := "atlas.sock"
	usr, err := user.Current()
	if err == nil && usr.Uid != "" {
		sock = fmt.Sprintf("Atlas-Agent-%s.sock", usr.Uid)
	}
...
// Server represents a Atlas-Agent server bound to a specific address.
type Server struct {
...
func (s *Server) installHandler() {
	...
	mux.HandleFunc("GET /v1/workspaces/{id}/agents", c.handleGetWorkspaceSubagents)
	mux.HandleFunc("POST /v1/workspaces/{id}/agents", c.handlePostWorkspaceSubagent)
