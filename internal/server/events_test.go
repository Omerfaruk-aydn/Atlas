...
// are what non-interactive clients (e.g. `Atlas-Agent run`) rely on to
...
// through the SSE envelope. Without these correlators, `Atlas-Agent run`
...
// "agent failed" (returns non-zero from `Atlas-Agent run`) from "agent
