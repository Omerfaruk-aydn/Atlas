...
// with reason=tool_use and used to terminate `atlas run` early via
...
// subscribers (e.g. a TUI watching workspace events while `atlas
...
// non-nil error from `atlas run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `atlas run` waits) exits cleanly rather
...
// queue scenario: `atlas run --continue` attaches to a session
