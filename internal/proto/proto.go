...
// UpdateAvailable is published when a newer Atlas-Agent release is detected
...
// completion contract (notably `Atlas-Agent run` in client/server mode)
...
// `Atlas-Agent run`, which may attach to a busy session whose currently
