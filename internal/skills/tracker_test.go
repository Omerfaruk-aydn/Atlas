...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (atlas-config).
	activeSkills := []*Skill{
		{Name: "atlas-config", Description: "Atlas config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("atlas-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via crush://...).
	tracker.MarkLoaded("atlas-config")
	require.True(t, tracker.IsLoaded("atlas-config"))
