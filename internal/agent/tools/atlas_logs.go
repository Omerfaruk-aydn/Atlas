...
//go:embed atlas_logs.md.tpl
var atlasLogsDescriptionTmpl []byte

var atlasLogsDescriptionTpl = template.Must(
	template.New("atlasLogsDescription").
		Parse(string(atlasLogsDescriptionTmpl)),
)

type atlasLogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func atlasLogsDescription() string {
	return renderTemplate(atlasLogsDescriptionTpl, atlasLogsDescriptionData{
...
type AtlasLogsParams struct {
	Lines int `json:"lines,omitempty" description:"Number of recent log entries to return (default 50, max 100)"`
}

func NewAtlasLogsTool(logFile string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		AtlasLogsToolName,
		atlasLogsDescription(),
		func(ctx context.Context, params AtlasLogsParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			result := runAtlasLogs(logFile, params)
			return fantasy.NewTextResponse(result), nil
		},
	)
}

// runAtlasLogs reads and formats the last N log entries from the given file.
func runAtlasLogs(logFile string, params AtlasLogsParams) string {
