...
// event into a closure nobody reads, and subscribers (`Atlas-Agent run`)
...
// publish a RunComplete for its RunID, hanging a `Atlas-Agent run` caller that
...
// than internal bookkeeping, which is the contract a `Atlas-Agent run` caller
...
// bookkeeping slice, not the published event a `Atlas-Agent run` caller awaits.
