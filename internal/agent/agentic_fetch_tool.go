...
func (c *coordinator) agenticFetchTool(_ context.Context, client *http.Client) (fantasy.AgentTool, error) {
	...
	return fantasy.NewParallelAgentTool(
		...
		func(ctx context.Context, params tools.AgenticFetchParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			...
			tmpDir, err := os.MkdirTemp(c.cfg.Config().Options.DataDirectory, "Atlas-Agent-fetch-*")
			if err != nil {
			...
			_, small, _, _, err := c.buildAgentModels(ctx, true)
			if err != nil {
			...
			webFetchTool := tools.NewWebFetchTool(tmpDir, client, urlPolicy(c.cfg))
			...
			fetchTools := []fantasy.AgentTool{
				...
				tools.NewViewTool(c.lspManager, c.permissions, c.filetracker, nil, tmpDir, tools.NewViewLimits(c.cfg.Config())),
			}
