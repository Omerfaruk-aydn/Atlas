...
func coderAgent(r *vcr.Recorder, env fakeEnv, large, small fantasy.LanguageModel) (SessionAgent, error) {
	...
	// independently of user config on `$HOME/.config/Atlas-Agent/atlas.json`.
	...
	cfg.Config().Options.DisabledSkills = []string{"atlas-config"}
	...
	allTools := []fantasy.AgentTool{
		...
		tools.NewDownloadTool(env.permissions, env.workingDir, r.GetDefaultClient(), tools.URLPolicy{}, tools.PathPolicy{}, 0),
		...
		tools.NewViewTool(nil, env.permissions, *env.filetracker, nil, env.workingDir, tools.ViewLimits{}),
