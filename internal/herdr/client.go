...
// Client reports Atlas-Agent agent state to a running herdr instance.
type Client struct {
	socketPath string
	paneID     string
...
// is not strictly greater. Because Atlas-Agent seeds seq from the wall
// clock at startup (see newFromEnv), a restarted Atlas-Agent in the same
...
func (c *Client) newRequestLocked(method, idPrefix, state string) reportRequest {
	...
	return reportRequest{
		ID:     fmt.Sprintf("Atlas-Agent:%s:%d", idPrefix, time.Now().UnixNano()),
		Method: method,
		Params: reportParams{
			PaneID:         c.paneID,
			Source:         "Atlas-Agent",
