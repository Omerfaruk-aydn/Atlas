...
type RunOptions struct {
	...
	// the Atlas-Agent process cwd — hooks and the bash tool have different
...
// Atlas-Agent handler stack. Shared by the stateless [Run] entrypoint and the
...
// standard Atlas-Agent middleware chain (builtins, script dispatch, block list)
...
// when sourcing framework files can send SIGINT/SIGTERM to Atlas-Agent's process
...
// regardless of the caller's environment because Atlas-Agent shells are never
...
//  1. builtins first (so Atlas-Agent's in-process jq wins over any PATH binary);
...
// builtinHandler returns middleware that dispatches recognized Atlas-Agent
