...
// loadAtlasAgentSh writes a Atlas-Agent.sh into an isolated project and loads it through
...
func loadAtlasAgentSh(t *testing.T, script string) *config.ConfigStore {
	t.Helper()
	store, err := loadAtlasAgentShErr(t, script)
	require.NoError(t, err)
	return store
}

// loadAtlasAgentShErr is loadAtlasAgentSh without asserting success, for cases that are
// expected to fail at load time.
func loadAtlasAgentShErr(t *testing.T, script string) (*config.ConfigStore, error) {
	t.Helper()
	...
	workDir := t.TempDir()
	dataDir := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(workDir, "atlasrc"), []byte(script), 0o644))

	return config.Load(workDir, dataDir, false)
}

func TestShellConfigPermissionsAllow(t *testing.T) {
	store := loadAtlasAgentSh(t, `permissions allow bash view`)
...
func TestShellConfigPermissionsAccumulateAndDedup(t *testing.T) {
	store := loadAtlasAgentSh(t, `permissions allow bash
...
func TestShellConfigPermissionsDeny(t *testing.T) {
	store := loadAtlasAgentSh(t, `permissions deny bash sourcegraph
