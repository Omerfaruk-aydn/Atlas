...
// isolateReloadEnv points HOME/XDG/ATLAS_AGENT_* at a throwaway directory so a
...
func isolateReloadEnv(t *testing.T) (workDir, dataDir string) {
	...
	t.Setenv("ATLAS_AGENT_GLOBAL_CONFIG", filepath.Join(isolated, ".config", "Atlas-Agent"))
	t.Setenv("ATLAS_AGENT_GLOBAL_DATA", filepath.Join(isolated, ".local", "share", "Atlas-Agent"))
	return t.TempDir(), t.TempDir()
}

// TestReloadFromDisk_PicksUpEditedatlasrc verifies that a atlasrc edit on
// disk is reflected in memory after a reload, matching how JSON config
// already behaves.
func TestReloadFromDisk_PicksUpEditedatlasrc(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "atlasrc")
	...
	require.Equal(t, "osc", store.Config().Options.Notifications,
		"reload must re-execute the atlasrc and pick up the new value")
}

// TestReloadFromDisk_FailingatlasrcKeepsOldConfig verifies that a atlasrc
...
func TestReloadFromDisk_FailingatlasrcKeepsOldConfig(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "atlasrc")
	require.NoError(t, os.WriteFile(rcPath, []byte("option notifications bell\n"), 0o644))
	...
	// Overwrite with a atlasrc that errors at load time (unknown option key).
	require.NoError(t, os.WriteFile(rcPath, []byte("option totally-bogus-key value\n"), 0o644))

	err = store.ReloadFromDisk(context.Background())
	require.Error(t, err, "a failing atlasrc must surface a reload error")
	require.Equal(t, "bell", store.Config().Options.Notifications,
		"in-memory config must be preserved when a reload fails")
}

// TestReloadFromDisk_HangingatlasrcIsInterruptible is the core regression test
// for the reload-path blocker: config reloads run while holding the store's
// write lock, so a atlasrc that blocks (busy loop, hung command substitution)
...
func TestReloadFromDisk_HangingatlasrcIsInterruptible(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "atlasrc")
	require.NoError(t, os.WriteFile(rcPath, []byte("option notifications bell\n"), 0o644))
	...
	// Overwrite with a atlasrc that never terminates.
...
// TestLoad_TracksNotYetCreatedGlobalatlasrc verifies that config.Load tracks
// the global atlasrc path even when the file does not exist yet, so a atlasrc
// created after startup is detected as a staleness change. Previously only
// successfully-loaded paths were tracked, so a mid-session global atlasrc went
// unnoticed until something else triggered a reload.
//
// Project-level atlasrc files are discovered by walking the tree for existing
// files, so a not-yet-created project atlasrc is still only picked up on the
...
func TestLoad_TracksNotYetCreatedGlobalatlasrc(t *testing.T) {
	...
	t.Setenv("ATLAS_AGENT_GLOBAL_CONFIG", filepath.Dir(globalRC))
