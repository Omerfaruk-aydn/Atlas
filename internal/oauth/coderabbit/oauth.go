// Package coderabbit implements the OAuth2 + PKCE login flow coderabbit
// Code's web IDE uses to authorize a local tool, so Atlas-Agent can
// use an coderabbit Pro/Enterprise subscription's flat-rate quota
// instead of a separate pay-per-token API key.
//
// This is a SCAFFOLD, not a finished integration. coderabbit does not
...
//
// Risk note: coderabbit's terms of service restrict the service to
// first-party clients. Using this login is at the user's own risk to
// their coderabbit account standing, not Atlas-Agent's.
package coderabbit
...
const (
	...
	// from a logged-in coderabbit Code session.
	clientID     = "REPLACE_WITH_REAL_coderabbit_OAUTH_CLIENT_ID"
	clientSecret = ""
	authorizeURL = "https://api.coderabbit.ai/oauth/authorize"
	tokenURL     = "https://api.coderabbit.ai/oauth/token"
	redirectPort = 41543
...
func Start(ctx context.Context) (*AuthSession, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("bind localhost:%d for coderabbit sign-in (is another login already running?): %w", redirectPort, err)
	}
...
func (s *AuthSession) WaitWithProgress(ctx context.Context, progress func(string)) (*oauth.Token, error) {
	...
	case res := <-s.resultCh:
		...
		// TODO(coderabbit): discover the account id and set tok.AccountID
		// / tok.PlanType here.
		return tok, nil
	case <-ctx.Done():
		return nil, ctx.Err()
