// Package zed implements the OAuth2 + PKCE login flow zed
// Code's web IDE uses to authorize a local tool, so Atlas-Agent can
// use an zed Pro/Enterprise subscription's flat-rate quota
// instead of a separate pay-per-token API key.
//
// This is a SCAFFOLD, not a finished integration. zed does not
...
//
// Risk note: zed's terms of service restrict the service to
// first-party clients. Using this login is at the user's own risk to
// their zed account standing, not Atlas-Agent's.
package zed
...
const (
	...
	// from a logged-in zed Code session.
	clientID     = "REPLACE_WITH_REAL_zed_OAUTH_CLIENT_ID"
	clientSecret = ""
	authorizeURL = "https://account.zed.dev/oauth/authorize"
	tokenURL     = "https://account.zed.dev/oauth/token"
	redirectPort = 41544
...
func Start(ctx context.Context) (*AuthSession, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("bind localhost:%d for zed sign-in (is another login already running?): %w", redirectPort, err)
	}
...
func (s *AuthSession) WaitWithProgress(ctx context.Context, progress func(string)) (*oauth.Token, error) {
	...
	case res := <-s.resultCh:
		...
		// TODO(zed): discover the account id and set tok.AccountID
		// / tok.PlanType here.
		return tok, nil
	case <-ctx.Done():
		return nil, ctx.Err()
