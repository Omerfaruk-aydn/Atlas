// Package windsurf implements the OAuth2 + PKCE login flow Codeium
// (the company behind Windsurf) exposes for its Windsurf Pro/Teams
// coding plan, so Atlas-Agent can use that subscription's flat-rate
// quota instead of a separate Codeium API key.
//
// This is a SCAFFOLD, not a finished integration. Codeium does not
// publish a public OAuth flow for Windsurf the way Google does for
// Antigravity. The client id, authorize/token endpoints, and the
// request envelope the Windsurf backend expects are not part of any
// official documentation and have to be observed against the real
// Windsurf IDE or browser console. The shape of this package is the
// same PKCE flow Antigravity ships so swapping in real values is a
// small change once they are known.
//
// Risk note: Codeium's terms of service restrict the service to
// first-party clients. Using this login is at the user's own risk to
// their Codeium account standing, not Atlas-Agent's.
//
// To finish wiring this up:
//  1. Capture the OAuth client id from a logged-in Windsurf session
...
//     internal/deps/atlas-llm/providers/windsurf.
package windsurf
...
const (
	...
	// from a logged-in Windsurf session.
	clientID     = "REPLACE_WITH_REAL_WINDSURF_OAUTH_CLIENT_ID"
	clientSecret = ""
	authorizeURL = "https://codeium.com/oauth/authorize"
	tokenURL     = "https://codeium.com/oauth/token"
	redirectPort = 41539
...
func Start(ctx context.Context) (*AuthSession, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("bind localhost:%d for Windsurf sign-in (is another login already running?): %w", redirectPort, err)
	}
