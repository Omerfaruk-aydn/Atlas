// Package factory implements the OAuth2 + PKCE login flow factory
// Code's web IDE uses to authorize a local tool, so Atlas-Agent can
// use an factory Pro/Enterprise subscription's flat-rate quota
// instead of a separate pay-per-token API key.
//
// This is a SCAFFOLD, not a finished integration. factory does not
...
//
// Risk note: factory's terms of service restrict the service to
// first-party clients. Using this login is at the user's own risk to
// their factory account standing, not Atlas-Agent's.
package factory
...
const (
	...
	// from a logged-in factory Code session.
	clientID     = "REPLACE_WITH_REAL_factory_OAUTH_CLIENT_ID"
	clientSecret = ""
	authorizeURL = "https://api.factory.ai/oauth/authorize"
	tokenURL     = "https://api.factory.ai/oauth/token"
	redirectPort = 41542
...
func Start(ctx context.Context) (*AuthSession, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("bind localhost:%d for factory sign-in (is another login already running?): %w", redirectPort, err)
	}
...
func (s *AuthSession) WaitWithProgress(ctx context.Context, progress func(string)) (*oauth.Token, error) {
	...
	case res := <-s.resultCh:
		...
		// TODO(factory): discover the account id and set tok.AccountID
		// / tok.PlanType here.
		return tok, nil
	case <-ctx.Done():
		return nil, ctx.Err()
