...
// ATLAS_AGENT_CALLBACK_PREVIEW to a directory and it writes each page state
// there so the result can be opened in a browser and eyeballed.
func TestWritePreview(t *testing.T) {
	dir := os.Getenv("ATLAS_AGENT_CALLBACK_PREVIEW")
	if dir == "" {
		t.Skip("set ATLAS_AGENT_CALLBACK_PREVIEW=<dir> to render preview pages")
	}
